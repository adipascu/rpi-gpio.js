...
function Gpio() {
  ...
  this.read = this.input = function (channel, cb /* err,value */) {
    ...
    fs.readFile(`${PATH}/gpio${pin}/value`, 'utf-8', (err, data) => {
      if (err) {
        return cb(err);
      }
      data = (`${data}`).trim() || '0';
  ...
  function setRaspberryVersion(cb) {
    ...
    fs.readFile('/proc/cpuinfo', 'utf8', (err, data) => {
      ...
      currentValidBcmPins = [];
      Object.keys(PINS[pinVersion]).forEach((pin) => {
  ...
  function listen(channel, onChange) {
    ...
    const poller = new Epoll(((err, innerfd, events) => {
      if (err) throw err;
...
function isExported(pin, cb) {
  fs.exists(`${PATH}/gpio${pin}`, exists => cb(null, exists));
}
